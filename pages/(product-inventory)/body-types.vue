...
import type { BodyType } from '@/components/body-types/data/schema'
import { onMounted, ref } from 'vue'
import { bodyTypesColumns } from '@/components/body-types/components/columns'
import DataTable from '@/components/body-types/components/DataTable.vue'

import { useApi } from '@/composables/useApi'
import { valueUpdater } from '@/lib/utils'

const bodyTypesData = ref<BodyType[]>([])
...
async function fetchData() {
  ...
  try {
    const response = await api('/body-types', { params })
    const responseData = response?.data
    if (!Array.isArray(responseData)) {
      throw new Error('Invalid Laravel Paginator response structure.')
    }

    bodyTypesData.value = responseData
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    <div class="flex flex-wrap items-end justify-between gap-2">
      <div>
        <h2 v-t="'body-type.title'" class="text-2xl font-bold tracking-tight" />
        <p v-t="'body-type.description'" class="text-muted-foreground" />
      </div>
    </div>

    <DataTable
      :columns="bodyTypesColumns" :data="bodyTypesData" :meta="{ onDataChanged }"
