...
import type { Brands } from '@/components/brands/data/schema'
import { onMounted, ref } from 'vue'
import { brandsColumns } from '@/components/brands/components/columns'
import DataTable from '@/components/brands/components/DataTable.vue'
...
async function fetchData() {
  ...
  try {
    const response = await api('/brands', { params })

    // Laravel paginator returns data at the root, with pagination info either at the root
    // or nested under `meta` depending on the transformer. Handle both shapes.
    const responseData = response?.data
    if (!Array.isArray(responseData)) {
      throw new Error('Invalid Laravel Paginator response structure.')
    }

    brandsData.value = responseData
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    <div class="flex flex-wrap items-end justify-between gap-2">
      <div>
        <h2 v-t="'brand.title'" class="text-2xl font-bold tracking-tight" />
        <p v-t="'brand.description'" class="text-muted-foreground" />
      </div>
    </div>

    <DataTable
      :columns="brandsColumns" :data="brandsData" :meta="{ onDataChanged }"
