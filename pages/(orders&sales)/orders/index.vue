...
import { ordersColumns } from '@/components/orders/components/columns'
...
async function fetchData() {
  ...
  const api = useApi()
  try {
    const response = await api('/orders', { params }) as any
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4 p-6">
    <div class="flex flex-wrap items-end justify-between gap-2">
      <div>
        <h2 class="text-2xl font-bold tracking-tight">{{ t('orders.title', 'Orders') }}</h2>
        <p class="text-muted-foreground">{{ t('orders.description', 'Manage and track all customer orders.') }}</p>
      </div>
    </div>

    <DataTable :columns="ordersColumns" :data="ordersData" :meta="{ onDataChanged }" :page-count="pageCount"
