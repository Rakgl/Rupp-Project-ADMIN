...
import type { ContentBlock } from '@/components/content-blocks/data/schema'
import { onMounted, ref } from 'vue'
import { contentBlockColumns } from '@/components/content-blocks/components/columns'
import DataTable from '@/components/content-blocks/components/DataTable.vue'

import { useApi } from '@/composables/useApi'
import { valueUpdater } from '@/lib/utils'

const contentBlocksData = ref<ContentBlock[]>([])
...
async function fetchData() {
  ...
  try {
    const response = await api('/content-blocks', { params })

    if (!response || !response.data || !response.meta) {
      throw new Error('Invalid Laravel Paginator response structure.')
    }

    contentBlocksData.value = response.data
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    <div class="flex flex-wrap items-end justify-between gap-2">
      <div>
        <h2 v-t="'contentBlocks.title'" class="text-2xl font-bold tracking-tight" />
        <p v-t="'contentBlocks.description'" class="text-muted-foreground" />
      </div>
    </div>
