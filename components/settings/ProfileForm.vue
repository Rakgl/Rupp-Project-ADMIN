...
<template>
  ...
  <form v-else class="space-y-8" @submit.prevent="onSubmit">
      ...
      <FormItem>
        ...
        <FormControl>
          <div class="flex items-center space-x-4">
            <div class="relative">
              <div v-if="!avatarPreview" :class="cn(
              ...
              <div v-else-if="avatarPreview && avatarPreview.type === 'image'"
                ...
                <img :src="avatarPreview.src" :alt="t('profile.picture.alt', { name: values.name })"
            ...
            <div class="flex flex-col space-y-2">
              <Button type="button" variant="outline" size="sm" :disabled="isUploading"
                class="transition-all duration-200 hover:scale-105" @click="showAvatarOptions = !showAvatarOptions">
                {{
              ...
              <Button v-if="values.image" type="button" variant="ghost" size="sm" :disabled="isUploading"
          ...
          <input ref="fileInputRef" type="file" accept="image/png, image/jpeg, image/gif, image/webp" class="hidden"
            :disabled="isUploading" @change="handleFileUpload">
        </FormControl>

        <div v-if="showAvatarOptions"
          ...
          <div class="space-y-6">
            <div class="space-y-3">
              ...
              <Button type="button" variant="outline" size="sm" :disabled="isUploading"
                ...
                <svg v-if="!isUploading" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
            ...
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-7 gap-2 lg:grid-cols-10 md:grid-cols-9 sm:grid-cols-8">
                <button v-for="color in predefinedFallbackColors" :key="color.name" type="button" :class="cn(
                  ...
                  :disabled="isUploading" :title="color.name" @click="selectFallbackColor(color)" />
              </div>
            </div>
          </div>
        </div>
    ...
    <FormField v-slot="{ componentField }" name="email">
      <FormItem>
        ...
        <FormControl>
          <Input type="email" :placeholder="t('profile.email.placeholder')" v-bind="componentField" />
        </FormControl>
    ...
    <FormField name="language">
      <FormItem>
        ...
        <Select :default-value="values.language" @update:model-value="(value) => setFieldValue('language', value)">
          <FormControl>
    ...
    <div class="flex justify-start gap-2 pt-4">
      ...
      <Button type="button" variant="outline" :disabled="isUploading" @click="
        () =>
          resetForm({
            values: {
              ...defaultFormValues,
  ...
  <ImageViewer :open="showImagePreview" :image-url="avatarPreview?.src" @update:open="showImagePreview = $event" />
</template>
