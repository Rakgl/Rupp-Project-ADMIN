...
function resetEditForm(data?: Partial<StoreData>) {
  ...
  if (editLogoPreviewUrl.value?.startsWith('blob:')) URL.revokeObjectURL(editLogoPreviewUrl.value);
...
function removeEditLogo() {
  ...
  editStoreData.value.logo_file = null;
  if (editLogoInput.value) editLogoInput.value.value = '';
  if (editStoreData.value.logo_url) editStoreData.value.delete_logo = true;
}
...
async function handleUpdateStore() {
  ...
  if (dataToSend.logo_file) formData.append('logo', dataToSend.logo_file);
...
watch(isEditDialogOpen, (isOpen) => {
  if (isOpen) {
    nextTick(() => {
      if (editActiveTab.value === 'location') initializeMap();
    });
  } else {
    destroyMap();
  }
    ...
    <Dialog v-model:open="isViewDialogOpen">
      <DialogContent class="rounded-lg bg-white shadow-xl sm:max-w-2xl dark:bg-neutral-900">
        ...
        <div
          ...
          class="custom-scrollbar max-h-[70vh] space-y-4 overflow-y-auto px-4 py-4"
        >
          <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
            <div class="md:col-span-1">
              <Label class="text-sm font-medium text-gray-500 dark:text-neutral-400">{{
                t('stores.dialog.view.label_logo')
              }}</Label>
              <div
                class="mt-2 flex h-40 w-40 items-center justify-center overflow-hidden rounded-lg border bg-gray-50 dark:bg-neutral-800"
              >
                <img
    ...
    <Dialog v-model:open="isEditDialogOpen">
      <DialogContent class="rounded-lg bg-white shadow-xl sm:max-w-4xl dark:bg-neutral-900">
        ...
        <div v-else>
          <div
            v-if="editStoreError"
            class="mx-4 my-3 rounded-md border bg-red-50 px-4 py-3 text-sm text-red-700"
          >
          ...
          <div class="px-4 py-3 md:px-6">
            <Tabs v-model="editActiveTab" :default-value="TABS_CONFIG[0].id" class="w-full">
              <TabsList
                class="grid w-full grid-cols-4 rounded-lg bg-gray-100 p-1 dark:bg-neutral-800"
              >
                <TabsTrigger
                    ...
                    <CardContent class="space-y-6 p-6">
                      <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                        ...
                        <div class="space-y-2">
                          ...
                          <div
                            class="flex aspect-square w-full cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed p-4 text-center hover:border-primary"
                    ...
                    <CardContent class="space-y-4 p-6">
                      <div>
                        ...
                        <div
                          ...
                          class="mt-1 h-64 w-full rounded-md border bg-gray-100"
                        />
                      </div>
