...
export const newsColumns: ColumnDef<News>[] = [
  ...
  {
    accessorKey: 'name.en', // Access nested EN property for sorting/filtering
    header: ({ column }) => {
      const { t } = useI18n()
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'text-sm truncate max-w-[300px]' }, name?.en || 'N/A')
    },
    enableSorting: false,
  },
  {
    accessorKey: 'description.en', // Access nested EN property
    header: ({ column }) => {
      const { t } = useI18n()
